...
const AquaUserDashbordLayout = ({ children, title, subtitle }) => {
  ...
  return (
    <div className="relative h-[100dvh] overflow-hidden bg-slate-50">
      ...
      <div className="mx-auto flex h-full min-h-0 w-full max-w-[1320px] items-start gap-4 px-3 py-4 sm:px-5 lg:gap-5">
        <AquaUserDashboardHeader />

        <main className="flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden pb-24 lg:pb-0">
          ...
          <section className="mx-auto min-h-0 w-full max-w-5xl flex-1 overflow-hidden">
            <div className="flex h-full min-h-0 w-full flex-col rounded-[28px] border border-slate-200 bg-white p-4 sm:p-6 lg:p-7">
              ...
              <div
                key={router.pathname}
                className="min-h-0 flex-1 overflow-y-auto pr-1 animate-[dashboard-tab-in_180ms_cubic-bezier(0.22,1,0.36,1)]"
              >
                {children}
              </div>
            </div>
