  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between p-4 bg-gradient-to-b from-black/70 to-transparent">
        ...
        <div className="flex items-center gap-2">
          {isProduct && typeof inStock === "boolean" && (
            <span
              className={`text-xs ${inStock ? "bg-green-500" : "bg-red-500"} text-white px-2 py-1 rounded-full`}
            >
      ...
      <div className="relative aspect-[9/16]">
        ...
        {/* Play Icon on Hover */}
        <div
          className={`absolute inset-0 flex items-center justify-center transition-opacity duration-300 ${showOverlay ? "opacity-100" : "opacity-0"}`}
        >
        ...
        {images.length > 1 && (
          <div className="absolute top-16 left-4 right-4 flex gap-1 z-20">
            {images.map((_, index) => (
              ...
              >
                <div
                  className={`h-full bg-white transition-all duration-300 ease-out ${currentImage === index ? "w-full" : "w-0"}`}
                />
              </div>
            ))}
          </div>
        ...
        {/* Content Overlay */}
        <div
          className={`absolute inset-0 bg-black/50 flex flex-col justify-end p-4 transition-opacity duration-300 ${showOverlay ? "opacity-100" : "opacity-0"}`}
        >
          {isProduct ? (
            <>
              <div className="mb-4">
            ...
            <>
              ...
              {timestamp && (
                <p className="text-white/70 text-xs mb-4">{timestamp}</p>
              )}
            </>
          )}
          ...
          <div className="flex items-center justify-between text-white mb-4">
            <div className="flex items-center gap-4">
              ...
              >
                <Heart
                  className={`w-6 h-6 ${isWishlisted ? "fill-red-500 text-red-500" : "text-white group-hover:scale-110"} transition-transform`}
                />
