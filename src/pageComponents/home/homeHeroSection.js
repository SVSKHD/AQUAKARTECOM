    ...
    <div className="relative overflow-hidden bg-gradient-to-br from-white via-slate-50 to-emerald-50">
      ...
      <div className="relative mx-auto mt-8 max-w-7xl px-4 pb-12 pt-10 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-2">
          <div className="relative order-2 lg:order-1">
            <div className="grid grid-cols-2 grid-rows-2 gap-4">
              {(data || []).slice(0, 4).map((category, index) => (
                <motion.div
                  key={category?.id || category?._id || category?.title || index}
                  initial={{
                    opacity: 0,
                    x: -12 * (index + 1),
                    y: -12 * (index + 1),
          ...
          >
            <AquaImage src={logo} alt="Aquakart" customClass="mx-auto max-w-[180px]" />
            ...
            <p className="mt-4 max-w-xl text-base text-slate-600 lg:max-w-md">
              Discover tailored softeners, filtration, and RO systems engineered for Indian water. From borewell to municipal supply, we keep every tap pristine and hassle-free.
            </p>
