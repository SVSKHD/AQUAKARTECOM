...
const AquaPaymentDetails = ({ paymentInstrument }) => {
  ...
  const { type, utr, cardNetwork, accountType, last4, expiry } = paymentInstrument;

  return (
    <div className="ml-4 mt-4">
      {type && <p className="text-gray-900">Type: {type}</p>}
      {utr && <p className="text-gray-900">UTR: {utr}</p>}
      {cardNetwork && <p className="text-gray-900">Card Network: {cardNetwork}</p>}
      {accountType && <p className="text-gray-900">Account Type: {accountType}</p>}
